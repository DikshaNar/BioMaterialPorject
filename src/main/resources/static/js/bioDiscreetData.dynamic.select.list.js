...
$('.addVariable').on('click', function() {
    ...
    var i = $("#number").val();
    if(i<11){
		$('.variableName:last').before('<div class="col-sm-1"><form:label  class="text-info text-infoo" path="yVariableId" >X-axis variable('+i+'): </form:label></div>');		
    	$('.block:last').before('<div class="col-sm-1"><select class="form-control" id="yVariableId" value="${bioDiscreetDataForm.yVariableId}"></select></div>');
	} else {
		$.alert({
				title: 'Alert!',
