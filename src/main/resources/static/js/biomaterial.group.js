...
function update() {
	...
	$.get("getBioCompositionByNutrientDescs?nutrientDesc=" + option.text, function(response) {
		var jsonValue = JSON.parse(response);
		if(count <= 4)
			$('.addComponents').append('<label class="text-info col-sm-1" >'+ option.text +' </label><div class="col-sm-1"><input class="form-control" id="'+ jsonValue.data.id +'" value=""/></div><label class="text-info col-sm-1" id="gram">'+ jsonValue.data.uom +'</label>');
		else 
			$('.addComponents'+x+'').append('<label class="text-info col-sm-1" >'+ option.text +' </label><div class="col-sm-1"><input class="form-control" id="'+ jsonValue.data.id +'" value=""/></div><label class="text-info col-sm-1" id="gram">'+ jsonValue.data.uom +'</label>');
...
$(document).ready(function() {   
	$("#saveCustomMaterial").on("click", function() { 
		...
		for (i = 0; i < additionalComponentsName.length; i++) {
			//componentsName.push(additionalComponentsName[i]);
...
function autoFillLongDesc() {
	...
	$("#selectedMaterialId").on("change", function(){		
        ...
        document.getElementById("longDesc").innerHTML = selectedMaterial.toLowerCase() + ","+ shortDesc.toLowerCase() + "," + keyword1.toLowerCase() + "," + keyword2.toLowerCase() + "," + keyword3.toLowerCase() + "," + keyword4.toLowerCase();
    });
    $('#shortDesc').keyup(function(e) {		
	    //if($("#selectedMaterialId").val() != ""){
			bioStatus = 0;
			shortDesc = $('#shortDesc').val();
			selectedMaterial = $("#selectedMaterialId").val();
			document.getElementById("longDesc").innerHTML = selectedMaterial.toLowerCase() + ","+ shortDesc.toLowerCase() + "," + keyword1.toLowerCase() + "," + keyword2.toLowerCase() + "," + keyword3.toLowerCase() + "," + keyword4.toLowerCase();
		/*} else {
			$.alert({
					title: 'Alert!',
					content: 'Please select Material first!',
				});
		}*/ 
	});	  	
	if(selectedMaterial != "")	
		...
		if(biomaterialId.includes("9999")){
			if(biomaterialId != null){
				...
				for(var i = 0; i<listOfCustomMaterial.customBioMaterialList.length; i++){
					customMaterialDetails += "<TR><TD>ID</TD><TD>"+listOfCustomMaterial.customBioMaterialList[i].id+"</TD></TR><TR><TD>Short Desc</TD><TD>"+listOfCustomMaterial.customBioMaterialList[i].shortDesc+"</TD></TR>"+
			...
			if(biomaterialId.includes("9999")){
				if(biomaterialId != null){
					...
					$.get("getCustomBioMaterialUsingId?biomaterialId=" + biomaterialId, function(response) {
						var listOfCustomMaterial = JSON.parse(response);								
						for(var i = 0; i<listOfCustomMaterial.customBioMaterialList.length; i++){	
							shortDesc = listOfCustomMaterial.customBioMaterialList[i].shortDesc;
						...
						$("#divTable").hide();
						$("#editHideDiv").show();	
						$.get("getBioCompositionByNutrientDesc?nutrientDesc=" + additionalComponentsName + "&existingCompositionValue=" + existingCompositionValue, function(response) {
							var jsonValue = JSON.parse(response);
							for (var k = 0; k <= 3; k++) {
							...
							if(jsonValue.data.length > 3){
								for (var k = 4; k < jsonValue.data.length; k++) {
									...
									$('.addComponents'+x+'').append('<label class="text-info col-sm-1" >'+ jsonValue.data[k].nutrientDesc +' </label><div class="col-sm-1"><input class="form-control" id='+ jsonValue.data[k].id +' value='+ jsonValue.value[k] +'></div><label class="text-info col-sm-1" id="gram">'+ jsonValue.data[k].uom +'</label>');									
									additionalComponentsId.push(jsonValue.data[k].id);		
								}
							}						
						});									
