...
public class DiscreetDataController {
	...
	public ModelAndView searchBioProperty() {
		...
		ModelAndView mv = new ModelAndView("contribute/searchBioProperty", "showBioDiscreetDataForm", showBioDiscreetDataForm);
		return mv;
	}
	
	@PostMapping("showBioProperty")
	private ModelAndView showBioProperty(HttpServletRequest request, @Valid @ModelAttribute ShowBioDiscreetDataForm showBioDiscreetDataForm, BindingResult bindingResult,@AuthenticationPrincipal Principal principal) {
		logger.info("Start");
		ModelAndView  mv ;
		List<BioDiscreetData> bioDiscreetData = null;
		if(bindingResult.hasErrors()) {
			...
			bioDiscreetData = bioDiscreetDataService.getBioDiscreetDataByDataSetNameORYVariable(showBioDiscreetDataForm.getDataSetName());
		}
