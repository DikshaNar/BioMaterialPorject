...
public interface BioMaterialRepository extends JpaRepository<BioMaterial, Long> {
		...
		@Transactional
		@Modifying
		@Query(value = "UPDATE bio_material SET short_desc =:shortDescName, long_desc=:longDescName, updated_by=:userName, updated_at=:updatedDate WHERE id=:materialId", nativeQuery = true)
		void updateCustomBioMaterialUsingId(@Param("materialId") Long materialId, @Param("shortDescName") String shortDescName, @Param("longDescName") String longDescName, @Param("updatedDate") String updatedDate, @Param("userName") String userName);
