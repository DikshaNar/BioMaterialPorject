...
<div class="container">
    ...
    <c:forEach var="bioMaterial" items="${bioMaterialSearchResultsForm.bioMaterials}">
      <tr>
        ...
        <td>
		    <button type="button" class="btn btn-default clickedBioMaterialPopup" id="clickedBioMaterialPopup">
		      				<span class="glyphicon glyphicon-search"></span> Details
		    			</button>&nbsp;&nbsp;&nbsp;<input type="hidden" value="${bioMaterial.id}" name="biomaterialId" id="biomaterialId"/>
		     <button type="button" class="btn btn-default editBioMaterial" id="editBioMaterial">
		      				<span class="glyphicon glyphicon-edit"></span> Edit
		    			</button>&nbsp;&nbsp;&nbsp;<input type="hidden" value="${bioMaterial.id}" name="editBiomaterialId" id="editBiomaterialId"/>
		    			<c:choose>
	 <c:when test ="${not empty pageContext.request.remoteUser}">
	 	<input type="hidden" value="${pageContext.request.userPrincipal.principal.bioUser.userRole}" id="userName"/>
	 </c:when>
 </c:choose>		    			
			     		<a href="deleteMaterialUsingMaterialId?materialId=${bioMaterial.id}&shortDesc=${bioMaterialSearchResultsForm.bioMaterialSearchForm.bioMaterialName }" role="button" class="btn btn-info">
			    	  		Delete
			     		</a>
        </td>
      </tr>
	...
	<div class="modal fade" id="customMaterial" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	<c:forEach var="bioMaterial" items="${bioMaterialSearchResultsForm.bioMaterials}">
		<div class="modal fade" id="biomaterialdetailpopup" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		  <div class="modal-dialog modal-lg" role="document" >
		    <div class="modal-content">
		      <div class="modal-header">
		        <h5 class="modal-title" id="exampleModalLabel">Bio-Material Details</h5>
	...
	<!-- DETAILS POPUP END -->
	<h3 class="bg-info text-info" id="hideHeader" style="display:none">Contribute Your
			Bio-Material ...</h3>
	...
	<div class="form-horizontal" id="editHideDiv" style="display:none">
				...
				<input type="hidden" id="customIdHidden" /> 
				<label class="text-info col-sm-1">Name</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" id="shortDesc"
								name="shortDesc" oninput="autoFillLongDesc()" required />
					...
					<h4 class="bg-info text-info">Composition(in gms out of
						100gms)</h4>
					<div class="form-group">
						<label class="text-info col-sm-1" id="waterLabel">Water </label>
						<div class="col-sm-1">
							<input class="form-control" id="water" value="" oninput="composition()" required />
						</div>
						<label class="text-info col-sm-1" id="waterGram">g</label>
						<label class="text-info col-sm-1" id="fatLabel">Fat </label>
						<div class="col-sm-1">
							<input class="form-control" id="fat" value="" oninput="composition()" required />
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-1">
							<input class="form-control" id="protein" value="" oninput="composition()" required />
						</div>
						<label class="text-info col-sm-1" id="proteinGram">g</label>
						<label class="text-info col-sm-1" id="fiberLabel">Fiber </label>
						<div class="col-sm-1">
							<input class="form-control" id="fiber" value="" oninput="composition()" required />
						</div>
						<label class="text-info col-sm-1" id="fiberGram">g</label>
						<label class="text-info col-sm-2">Additional Components:-</label>
					...
					<div class="form-group">
						...
						<div class="col-sm-1">
							<input class="form-control" id="carb" value="" oninput="composition()" required />
						</div>
						<label class="text-info col-sm-1" id="carbGram">g</label>
						<label class="text-info col-sm-1" id="ashLabel">Ash </label>
						<div class="col-sm-1">
							<input class="form-control" id="ash" value="" oninput="composition()" required />
						</div>
